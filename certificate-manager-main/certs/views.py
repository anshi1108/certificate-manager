...
def certificate_detail(request, certificate_id):
    ...
    if USE_DIGICERT_API:
        ...
        files_data = {
            'private_key': str(certificate.private_key).replace('private_keys/', '') if certificate.private_key else None,
            'certificate': str(certificate.certificate).replace('certificates/', '') if certificate.certificate else None,
        }
...
def download_file(request, certificate_id, file_type):
    """
    Allows the user to download the private key or certificate for a given certificate_id.
    """
    try:
        # Fetch the certificate object from the database
        certificate = Certificate.objects.get(id=certificate_id)

        # Determine the file path based on the requested file type
        if file_type == "certificate" and certificate.certificate:
            file_path = os.path.join(settings.MEDIA_ROOT, certificate.certificate.name)
        elif file_type == "private_key" and certificate.private_key:
            file_path = os.path.join(settings.MEDIA_ROOT, certificate.private_key.name)
        else:
