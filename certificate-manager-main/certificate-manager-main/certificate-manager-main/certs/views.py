...
def certificate_list(request):
    # Check the role in the session and redirect accordingly
    ...
    # Default: if the role is not found or is not valid, return the certificate list
...
def cc_admin_users(request):
    # Load the users from the credentials file
    credentials = load_credentials()
    users = credentials.get('website_admins', [])
    return render(request, 'cc_admin_users.html', {'users': users})
